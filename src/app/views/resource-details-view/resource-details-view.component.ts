...
export class ResourceDetailsView implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.urlData.resourceType = <string>this.route.snapshot.paramMap.get("type");
    this.urlData.resourceId = <string>this.route.snapshot.paramMap.get("id");
    //Get current resource data from the API
    this.getResourceDetailsByNameAndId(this.urlData.resourceType, this.urlData.resourceId);
    //Get Session
  ...
  findExistInUserSavedItems(){
    ...
    this.savedItemService.findJikanItemExistanceInSavedItems(Number(this.urlData.resourceId))
      .subscribe({
